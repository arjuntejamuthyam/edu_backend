...
@app.put("/admin")
def update_admin(body: AdminUpdateIn, db: Session = Depends(get_db)):
    admin = db.query(AdminRow).filter_by(id=body.currentId, password=body.currentPassword).first()
    if not admin:
        raise HTTPException(status_code=401, detail="Current ID or password is incorrect")
    if body.newId != body.currentId and db.query(AdminRow).filter_by(id=body.newId).first():
        raise HTTPException(status_code=400, detail="That ID is already taken")
    # ID is the primary key, so if it's changing we delete + recreate the row.
    db.delete(admin)
    db.flush()
    db.add(AdminRow(id=body.newId, password=body.newPassword, name=admin.name))
    db.commit()
    return {"ok": True}
